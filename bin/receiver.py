...
def checkCert(cur_config, minerva_core):
    ...
    if len(cur_config['listen_ip'].keys()) == 1 and cur_config['listen_ip'].keys()[0] == '0.0.0.0':
        for p in cur_config['listen_ip']['0.0.0.0']['rec_ports']:
            certdb.update({"SERVER": "receiver"}, { "$addToSet": { "receivers": "%s-%i-%i" % (platform.node(), p, cur_config['listen_ip']['0.0.0.0']['pub_port']) }})
    else:
